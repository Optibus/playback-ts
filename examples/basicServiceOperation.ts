...
class ServiceOperation {
    ...
    private getRequestData(rangeMax: number) {
        ...
        const getData = tapeRecorder.interceptInput({
            alias: "getRequestData",
            func: (max) => Math.floor(Math.random() * max) + 1
        })
        return getData(rangeMax);
    }
    ...
    private storeResult(result: any) {
        const store = tapeRecorder.interceptOutput({
            alias: "output_function",
            func: (value) => {
                return this.putResultInMongo(value);
            },
        });
        return store(result);
    }
